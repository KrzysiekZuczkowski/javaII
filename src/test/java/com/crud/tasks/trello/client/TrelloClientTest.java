...
class TrelloClientTest {
    ...
    public void shouldCreateCard() throws URISyntaxException {
        ...
        when(trelloConfig.getTrelloAppKey()).thenReturn("test");
        when(trelloConfig.getTrelloToken()).thenReturn("test");
        TrelloCardDto trelloCardDto = new TrelloCardDto("test name",
                "test description", "top", "test_idList"
        );

        URI uri = new URI("http://test.com/cards?key=test&token=test&name=test%20name" +
                "&desc=test%20description&pos=top&idList=test_idList");
